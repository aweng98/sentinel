  ...
  "Resources" should {
    "be easy to store in the DB" in new WithControllerAndRequest {
      ...
      val request = fakeRequest("POST", "/asset ").
        ...
        withHeaders(
          "Authorization" -> s"username=${owner.getCredentials.username};api-key=${
            owner
      ...
      // Disabling the check, until we find the root cause
    ...
    "be safe to store in bulk" in new WithControllerAndRequest {
      ...
      val requests = for (user <- newUsers) yield {
        // Assets must have unique names, or an error is thrown.
        val assetName = s"${user.getCredentials.username}/ice-cream"
        fakeRequest("POST", "/asset ").
          withJsonBody(Json.parse( s"""{"name": "${assetName}", "owner": "${user.id.get}"}"""))
      }
